...
const useGetState = <TState, TSelectedState>(
    ...
    const fnAreEqual: TAreEqual<TSelectedState> = areEqual ?? configuration.areEqual;
    ...
    const ref = useRef<{ state: TSelectedState }>({ state: fnSelector(cell.getState()) });
    const [state, setState] = useState<TSelectedState>(fnSelector(cell.getState()));
